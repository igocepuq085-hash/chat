...
const dailyActivity = [
  { label: '19 мая', value: 1 }, { label: '20 мая', value: 31 }, { label: '21 мая', value: 14 }, { label: '22 мая', value: 12 },
  { label: '23 мая', value: 12 }, { label: '24 мая', value: 6 }, { label: '25 мая', value: 8 }, { label: '26 мая', value: 3 },
  { label: '27 мая', value: 26 }, { label: '28 мая', value: 23 }, { label: '29 мая', value: 2 }, { label: '31 мая', value: 3 },
  { label: '1 июн.', value: 8 }, { label: '2 июн.', value: 5 },
];
...
const sortLabels: Record<SortKey, string> = { score: 'Индекс', messages: 'Сообщения', reactions: 'Реакции', activeDays: 'Дни', threads: 'Инициатива', quickResponses: 'Ответы' };
...
function ActivityHeatmap() {
  ...
  return <div className="heatmap" aria-label="Активность по часам">{hourlyActivity.map((item) => { const intensity = item.value / max; return <div className="heat-cell" key={item.label}><span style={{ opacity: 0.25 + intensity * 0.75, transform: `scale(${0.72 + intensity * 0.28})` }} /><strong>{item.label}</strong><small>{item.value}</small></div>; })}</div>;
}
